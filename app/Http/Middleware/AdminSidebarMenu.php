...
class AdminSidebarMenu
{
            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('product.create'))
    $sub->url(
        action([\App\Http\Controllers\ProductOfferController::class, 'index']),
        __('product_offers.product_offers'),
        [
            'icon' => 'fa fas fa-tags',
    ...
    $sub->url(
        ...
        __('طباعة الباركود'),
        [
            'icon' => 'fa fa-barcode', // أيقونة الباركود
            'active' => request()->segment(1) == 'print-barcode'
        ]
    );
}
  ...
  <path d="M18 15v6" />
           ...
           if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
            ...
            if (in_array('stock_adjustment', $enabled_modules) && (auth()->user()->can('stock_adjustment.view') || auth()->user()->can('stock_adjustment.create') || auth()->user()->can('view_own_stock_adjustment'))) {
                $menu->dropdown(
                    __('stock_adjustment.stock_out_quantity'),
                    function ($sub) {
...
//if (auth()->user()->can('tax_invoice_report.view')) {
  //  $sub->url(
    //    action([\App\Http\Controllers\JoReportController::class, 'index']),
      //  __('jo_reports.tax_invoice_report'),
        //[
          //  'icon' => 'fa fas fa-file-invoice-dollar',
            //'active' => request()->is('tax-invoices-report')
        //]
    //);
//}
                        ...
                        if (auth()->user()->can('report.quantity_entry_report')) {
                            $sub->url(
                                // Route in controller 
                                action([\App\Http\Controllers\ReportController::class, 'quantityEntryReport']),
                                // For translate 
                                __('report.quantity_entry_report'),
                                [
                                    'icon' => '','active' => request()->segment(1) == 'quantity-entry-report'
                                ] 
                            );
                        }
                        ////////////////////////////////////////////// quantity entry report 001
                        ...
                        if (auth()->user()->can('report.product_sales_detailed_report')) {
                            $sub->url(
                                ...
                                // For translate 
                                __('sales_detailed.sales_returns_report'),
                                [
                                    'icon' => '','active' => request()->segment(1) == 'product-sales-detailed-report'
                                ] 
                            );
                        }
                        ////////////////////////////////////////////// product_sales_detailed_report 001
                        ...
                        if ($is_admin) {
                            $sub->url(
                                ...
                                ['icon' => '', 'active' => request()->segment(2) == 'getPaymentMethodReport']
                            );
                        } /////////////////////// 008
...
if (auth()->user()->can('daily_sales_report.view')) {  //001
    $sub->url( 
        ...
        [
            ...
            'active' => request()->segment(2) == 'daily-sales-report'
        ]
    );
}
...
//     $sub->url(
//         action([\App\Http\Controllers\EgoInvoiceController::class, 'index']),
//         __('report.invoice_statement'),
//         [
//             'icon' => 'fa fa-file-text-o',
//             'active' => request()->segment(1) == 'invoice-statement'
//         ]
//     );
...
// }
           ...
           if (auth()->user()->can('barcode_design.access')) {
    $sub->url(
        action([\App\Http\Controllers\BarcodeDesignController::class, 'index']),
        __('barcode.barcode_design'),
        [
            'icon' => '', 
            'active' => request()->segment(1) == 'barcode-design'
        ]
    );
}
