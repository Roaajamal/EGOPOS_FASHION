...
public function getPaymentMethodReport(Request $request)
{
    ...
    if ($request->ajax()) {
        try {
            $location_id = $request->get('location_id');
           $full_start = $request->get('start_date');
$full_end = $request->get('end_date');

if (!empty($full_start) && !empty($full_end)) {
    // إذا تم إرسال التاريخ بدون وقت (مثلاً 10-10-2023) سنحوله لبداية ونهاية اليوم
    if (strlen($full_start) <= 10) { 
        $full_start = \Carbon\Carbon::parse($full_start)->startOfDay()->toDateTimeString();
    }
    if (strlen($full_end) <= 10) {
        $full_end = \Carbon\Carbon::parse($full_end)->endOfDay()->toDateTimeString();
    }
} else {
    // الحالة الافتراضية: اليوم كاملاً
    $full_start = \Carbon\Carbon::now()->startOfDay()->toDateTimeString();
    $full_end = \Carbon\Carbon::now()->endOfDay()->toDateTimeString();
}
            $total_cash_received = 0;
            $total_card_received = 0;
            $total_transactions_count = 0; 
            $payment_data = [];

            // 1. حساب المقبوضات (كاش وبطاقة)
            $methods = ['cash' => 'نقدي (Cash)', 'card' => 'بطاقة (Card)'];
            foreach ($methods as $key => $label) {
                $payment_q = \App\TransactionPayment::join('transactions as t', 'transaction_payments.transaction_id', '=', 't.id')
                    ->where('t.business_id', $business_id)
                    ->where('t.type', 'sell')
                    ->where('transaction_payments.method', $key);

                if ($location_id) $payment_q->where('t.location_id', $location_id);
                if ($full_start && $full_end) {
                    $payment_q->whereBetween('transaction_payments.paid_on', [$full_start, $full_end]);
                }

                $res = $payment_q->selectRaw("SUM(transaction_payments.amount) as total_amount, COUNT(transaction_payments.id) as total_count")->first();
                
                $amount = (float)($res->total_amount ?? 0);
                $count = (int)($res->total_count ?? 0);
                ...
                if ($key == 'cash') $total_cash_received = $amount;
                if ($key == 'card') $total_card_received = $amount;
                $total_transactions_count += $count;
            }

            // 2. حساب نقد الدرج (Initial Cash)
            $initial_cash_q = \DB::table('cash_register_transactions')
                ->join('cash_registers', 'cash_register_transactions.cash_register_id', '=', 'cash_registers.id')
                ->where('cash_registers.business_id', $business_id)
                ->where('cash_register_transactions.transaction_type', 'initial');

            if ($location_id) $initial_cash_q->where('cash_registers.location_id', $location_id);
            if ($full_start && $full_end) {
                $initial_cash_q->whereBetween('cash_register_transactions.created_at', [$full_start, $full_end]);
            }

            $initial_res = $initial_cash_q->selectRaw("SUM(amount) as total_amount, COUNT(cash_register_transactions.id) as total_count")->first();
            $total_initial_cash = (float)$initial_res->total_amount;
            $total_initial_count = (int)$initial_res->total_count;

            // 3. مبيعات الذمم
          $due_q = \App\Transaction::where('t.business_id', $business_id)
          ->from('transactions as t')
          ->where('t.type', 'sell')
          ->where('t.status', 'final')
          ->whereIn('t.payment_status', ['partial', 'due']);
          if ($location_id) $due_q->where('location_id', $location_id);
          if ($full_start && $full_end) {
          // نستخدم transaction_date لأنه التاريخ الرسمي للفاتورة في النظام
          $due_q->whereBetween('transaction_date', [$full_start, $full_end]);
          }
         $due_res = $due_q->selectRaw("
          COUNT(t.id) as total_count,
          SUM(t.final_total) as total_invoice_amount,
          SUM((SELECT COALESCE(SUM(tp.amount), 0) FROM transaction_payments tp WHERE tp.transaction_id = t.id)) as total_paid_amount
          ")->first();          
           $total_invoice_amount = (float)($due_res->total_invoice_amount ?? 0);
           $total_paid_amount = (float)($due_res->total_paid_amount ?? 0);
           $total_due = $total_invoice_amount - $total_paid_amount;

           $total_due_count = (int) ($due_res->total_count ?? 0);
           $total_transactions_count += $total_due_count;
           ...
           'total_amount' => $total_due
            ...
            // 4. المرتجعات النقدية
            $ret_q = \App\TransactionPayment::join('transactions as t', 'transaction_payments.transaction_id', '=', 't.id')
                ->where('t.business_id', $business_id)
                ->where('t.type', 'sell_return')
                ->where('transaction_payments.method', 'cash');

            if ($location_id) $ret_q->where('t.location_id', $location_id);
            if ($full_start && $full_end) $ret_q->whereBetween('transaction_payments.paid_on', [$full_start, $full_end]);

            $ret_res = $ret_q->selectRaw("SUM(transaction_payments.amount) as total_amount, COUNT(transaction_payments.id) as total_count")->first();
            $total_return_paid_cash = (float)$ret_res->total_amount;
            $total_return_count = (int)$ret_res->total_count;

            // 5. المصاريف النقدية
            $exp_q = \App\TransactionPayment::join('transactions as t', 'transaction_payments.transaction_id', '=', 't.id')
                ->where('t.business_id', $business_id)
                ->where('t.type', 'expense')
                ->where('transaction_payments.method', 'cash');

            if ($location_id) $exp_q->where('t.location_id', $location_id);
            if ($full_start && $full_end) $exp_q->whereBetween('transaction_payments.paid_on', [$full_start, $full_end]);

            $exp_res = $exp_q->selectRaw("SUM(transaction_payments.amount) as total_amount, COUNT(transaction_payments.id) as total_count")->first();
            $total_cash_expenses = (float)$exp_res->total_amount;
            $total_expenses_count = (int)$exp_res->total_count;

            // --- حساب تفاصيل المرتجعات (الجدول الثاني) ---
            $return_methods = ['cash' => 'مرتجع نقدي', 'card' => 'مرتجع بطاقة', 'due' => 'مرتجع  ( غير مدفوع)'];
            $returns_detail_data = [];

            foreach ($return_methods as $key => $label) {
                $ret_detail_q = \App\Transaction::where('business_id', $business_id)->where('type', 'sell_return');
                if ($location_id) $ret_detail_q->where('location_id', $location_id);
                if ($full_start && $full_end) $ret_detail_q->whereBetween('transaction_date', [$full_start, $full_end]);

                if ($key == 'due') {
                    $ret_detail_q->where('payment_status', '!=', 'paid');
                } else {
                    $ret_detail_q->where('payment_status', 'paid')
                        ->whereHas('payment_lines', function($q) use ($key) {
                            $q->where('method', $key);
                        });
                }
                $res_ret = $ret_detail_q->selectRaw("COUNT(id) as total_count, SUM(final_total) as total_amount")->first();
                $returns_detail_data[] = [
                    'method_label' => $label,
                    'total_count' => (int)$res_ret->total_count,
                    'total_amount' => (float)($res_ret->total_amount ?? 0)
                ];
            }

            // المعادلات النهائية
            $grand_total_combined = $total_cash_received + $total_card_received + $total_due; 
            $final_net_cash = ($total_cash_received + $total_initial_cash) - $total_cash_expenses - $total_return_paid_cash;

            return datatables()->of($payment_data)
                ->with([
                    'grand_total_combined' => $grand_total_combined,
                    'total_transactions_count' => $total_transactions_count,
                    'total_return_paid' => $total_return_paid_cash,
                    ...
                    'final_net_cash' => $final_net_cash,
                    'pure_data' => $payment_data,
                    'returns_details' => $returns_detail_data 
                ])
                ->make(true);
    ...
    return view('report.payment_method_report', compact('business_locations'));
} /////////////////  001 
    /**
    ...
    public function quantityEntryReport(Request $request)
{
    ...
    if ($request->ajax()) {
        ...
        // 1. الاستعلام الأساسي (الجداول المشتركة)
        ...
        // 2. تطبيق الفلاتر (يجب أن تكون قبل الـ Select والـ GroupBy)
        if (!empty($request->input('location_id'))) {
        ...
        if ($view_type == 'summary') {
            $query->join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
                ->select(
                    ...
                    'transactions.final_total as row_total_price',
                    DB::raw('NULL as sku'),
                    DB::raw('NULL as product_name'),
                    DB::raw('NULL as prev_qty'),
                    DB::raw('NULL as current_qty')
                )
            ...
            $query->join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
                ...
                ->select(
                    ...
                    'v.sub_sku as sku',
                    'pl.quantity as added_qty',
                    'pl.previous_quantity as prev_qty',
                    DB::raw('(pl.purchase_price * pl.quantity) as row_total_price'),
                    'bl.name as location_name',
                    'u.username as added_by'
                );
        }

        // 4. بناء الـ DataTable
        $datatable = Datatables::of($query)
            ->editColumn('transaction_date', '{{@format_datetime($transaction_date)}}')
            ->editColumn('added_qty', function($row) {
                return '<span class="display_currency" data-currency_symbol="false" data-orig-value="' . $row->added_qty . '">' . 
                $this->productUtil->num_f($row->added_qty, false) . '</span>';
            })
            ->editColumn('row_total_price', function($row) {
                return '<span class="display_currency" data-currency_symbol="true" data-orig-value="' . $row->row_total_price . '">' . 
                $this->productUtil->num_f($row->row_total_price, true) . '</span>';
            });

        // 5. منطق البحث
        if (!empty($request->input('search.value'))) {
            $search_term = $request->input('search.value');
            $datatable->filter(function ($q) use ($search_term, $view_type) {
                $q->where(function($sub_q) use ($search_term, $view_type) {
                    $sub_q->where('transactions.ref_no', 'like', "%{$search_term}%")
                          ->orWhere('u.username', 'like', "%{$search_term}%");
                    
                    if ($view_type == 'detailed') {
                        $sub_q->orWhere('p.name', 'like', "%{$search_term}%")
        ...
        if ($view_type == 'detailed') {
            $datatable->editColumn('prev_qty', function($row) {
                return $this->productUtil->num_f($row->prev_qty ?? 0, false);
            })
            ->addColumn('current_qty', function($row) {
                $current = ($row->prev_qty ?? 0) + $row->added_qty;
                return $this->productUtil->num_f($current, false);
            });
        }

        return $datatable->rawColumns(['transaction_date', 'added_qty', 'row_total_price'])
                         ->make(true);
    }

    $business_locations = BusinessLocation::forDropdown($business_id);
    return view('report.quantity_entry_report', compact('business_locations'));
}////////////001
    /**
    ...
    public function getSalesRepresentativeReport(Request $request)
    {
     ...
     /////////////////// لاظهار جميع المستخدمين في جدول واحد 001
public function getSalesRepresentativeSummary(Request $request) {
    ...
    $query = \App\User::where('business_id', $business_id);
    // إذا تم اختيار فرع معين (Location ID)
    // فلترة الموظفين حسب الفرع المختار (مباشرة من جدول users)
    ...
    foreach ($users as $user) {
        ...
        $commission_percent = (float)($user->cmmsn_percent ?? 0);
        $net_sales = (float)$total_sell - (float)$total_return;

        $data[] = [
            'name' => $user->first_name . ' ' . $user->last_name,
            'total_sell' => (float)$total_sell,
            'total_sell_return' => (float)$total_return,
            'total_expense' => (float)$total_expense,
            'net_sales' => $net_sales,
            'commission_percentage' => $commission_percent,
            'total_commission' => ($net_sales * $commission_percent) / 100,
        ];
    }
...
if (!empty($full_start) && !empty($full_end)) {
    // استخدم whereBetween مباشرة لأن البيانات ستصل بالوقت (24 ساعة) من الـ JS
    $query->whereBetween('transaction_payments.paid_on', [$full_start, $full_end]);
} else {
    // الحالة الافتراضية: اليوم كاملاً
    $query->whereBetween('transaction_payments.paid_on', [
        \Carbon\Carbon::now()->startOfDay()->toDateTimeString(),
    ...
    public function activityLog()
    {
        ...
        $transaction_types = [
           'contact' => __('report.contact'),
            ...
            'add_quantity' => __('quantity_entry.quantity_entry'),
            ...
            'stock_adjustment' => __('stock_adjustment.stock_adjustments'),
     ...
     // عرض الرقم المرجعي إذا وجد
     if (! empty($row->subject->ref_no)) {
        $html .= __('purchase.ref_no').': '.$row->subject->ref_no.'<br>';
     }

     // عرض رقم الفاتورة إذا وجد
     if (! empty($row->subject->invoice_no)) {
        $html .= __('sale.invoice_no').': '.$row->subject->invoice_no.'<br>';
    }

    // التعديل هنا: أضفنا 'quantity_entry' للمصفوفة ليعامل مثل البيع والشراء في عرض التفاصيل
    if ($row->subject_type == \App\Transaction::class && ! empty($row->subject) && in_array($row->subject->type, ['sell', 'purchase', 'quantity_entry'])) {
...
public function getDailySalesDetailsModal(Request $request) {
    ...
    return view('report.partials.daily_details_modal')->with(compact('sales', 'date'));
}
...
public function productAllSummaryReport(Request $request) {
    $business_id = $request->session()->get('user.business_id');
    if ($request->ajax()) {
        // استخدام الاسم المستعار 't' لجدول transactions لتوحيد الاستعلام
        $query = Transaction::from('transactions as t')
            ...
            ->select([
                ...
                'c.name as customer_name',
                't.type', 
                DB::raw("IF(t.type = 'sell_return', -1 * t.total_before_tax, t.total_before_tax) as final_total"),
                't.payment_status', 
                'bl.name as location_name'
            ]);
        ...
        if (!empty($request->start_date) && !empty($request->end_date)) {
            ...
            $end = \Carbon\Carbon::parse($request->end_date)->toDateTimeString();
        ...
        return Datatables::of($query)
            ...
            // منع البحث التلقائي في التاريخ لتجنب خطأ LOWER
            ->filterColumn('transaction_date', function($query, $keyword) {})
...
/////// تقرير مبيعات مجمل
public function productSalesSummaryReport(Request $request) {
    $business_id = $request->session()->get('user.business_id');
    if ($request->ajax()) {
        // استخدام الاسم المستعار t لتفادي مشاكل الأسماء الطويلة مع DataTables
        $query = Transaction::from('transactions as t')
            ->join('contacts as c', 't.contact_id', '=', 'c.id')
            ->join('business_locations as bl', 't.location_id', '=', 'bl.id')
            ->join('transaction_sell_lines as tsl', 't.id', '=', 'tsl.transaction_id')
            ->leftJoin('cash_register_transactions as crt', 't.id', '=', 'crt.transaction_id')
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
            ->where('t.status', 'final')
            ->select([
                ...
                't.final_total as total_line_amount', 
                't.payment_status',
                DB::raw("GROUP_CONCAT(DISTINCT crt.pay_method SEPARATOR ', ') as payment_methods"),
                'bl.name as location_name'
            ])
            ->groupBy('t.id');
        ...
        return Datatables::of($query)
            ...
            // منع البحث في الأعمدة التي تحتوي على SUM لعدم دعمها في WHERE (تحتاج HAVING)
            ->filterColumn('original_price', function($query, $keyword) {})
            ->filterColumn('unit_tax', function($query, $keyword) {})
            ->filterColumn('transaction_date', function($query, $keyword) {})
...
// 2. دالة التقرير  المبيعات التفصيلي (الأصناف مع كافة التفاصيل الـ 14)
public function productSalesDetailedReport(Request $request) {
    $business_id = $request->session()->get('user.business_id');
    if ($request->ajax()) {
        $query = TransactionSellLine::join('transactions as t', 'transaction_sell_lines.transaction_id', '=', 't.id')
            ...
            ->select([
                ...
                'p.name as product_name',
                'transaction_sell_lines.quantity',
                'transaction_sell_lines.unit_price_before_discount as price_before_tax',
                'transaction_sell_lines.unit_price_inc_tax as price_after_tax',
                DB::raw("(transaction_sell_lines.quantity * transaction_sell_lines.unit_price_inc_tax) as total_line_amount"),
                't.final_total as invoice_total',
                'bl.name as location_name',
                // استخدام subqueries لجلب الدفعات
        ...
        return Datatables::of($query)
            ...
            // منع البحث في الأعمدة التي تسبب أخطاء SQL
            ->filterColumn('transaction_date', function($query, $keyword) {})

            ->editColumn('price_before_tax', '{{@num_format($price_before_tax)}}')
            ->editColumn('price_after_tax', '{{@num_format($price_after_tax)}}')
            ->editColumn('total_line_amount', '{{@num_format($total_line_amount)}}')
            
            ->addColumn('cash_amount', function($row) {
                $ratio = $row->invoice_total > 0 ? ($row->total_line_amount / $row->invoice_total) : 0;
                return number_format($row->total_cash * $ratio, 2, '.', '');
            })
            ->addColumn('card_amount', function($row) {
                $ratio = $row->invoice_total > 0 ? ($row->total_line_amount / $row->invoice_total) : 0;
                return number_format($row->total_card * $ratio, 2, '.', '');
            })
...
// 1. دالة المرتجعات المجملة (فواتير المرتجع)
public function productReturnsSummaryReport(Request $request) {
    $business_id = $request->session()->get('user.business_id');
    if ($request->ajax()) {
        $query = DB::table('transactions as t')
            ->leftJoin('contacts as c', 't.contact_id', '=', 'c.id')
        ...
        return Datatables::of($query)
            ...
            // تعطيل البحث في الأعمدة التي تسبب أخطاء SQL
            ->filterColumn('transaction_date', function($query, $keyword) {})
            ->filterColumn('payment_methods', function($query, $keyword) {})

            ->editColumn('payment_status', function($row) {
                $status_labels = [
                    'paid' => '<span class="label bg-light-green">مدفوع</span>',
                    'due' => '<span class="label bg-red">مستحق</span>',
                    'partial' => '<span class="label bg-blue">جزئي</span>'
                ];
                return $status_labels[$row->payment_status] ?? $row->payment_status;
            })
            ->editColumn('payment_methods', function($row) {
                if (empty($row->payment_methods)) return '—';
                $methods_map = ['cash' => 'نقدي', 'card' => 'شبكة', 'cheque' => 'شيك', 'bank_transfer' => 'تحويل', 'other' => 'أخرى'];
                return implode(', ', array_map(fn($m) => $methods_map[$m] ?? $m, explode(', ', $row->payment_methods)));
            })
...
// 2. دالة المرتجعات التفصيلية (أصناف المرتجع)
public function productReturnsDetailedReport(Request $request) {
    $business_id = $request->session()->get('user.business_id');
    
    if ($request->ajax()) {
        $query = DB::table('transactions as t')
            ->leftJoin('transactions as parent', 't.return_parent_id', '=', 'parent.id')
            ->join('transaction_sell_lines as tsl', 't.return_parent_id', '=', 'tsl.transaction_id')
            ->whereRaw('tsl.quantity_returned > 0') 
        ...
        return Datatables::of($query)
            ...
            // --- إصلاح البحث لمنع خطأ LOWER وتوجيه الحقول للأصل ---
            ->filterColumn('invoice_no', function($query, $keyword) {
                $query->where('t.invoice_no', 'like', "%{$keyword}%");
            })
            ->filterColumn('customer_name', function($query, $keyword) {
            ...
            // منع البحث التلقائي في التاريخ لتجنب تعارض SQL
            ->filterColumn('transaction_date', function($query, $keyword) {})

            ->editColumn('quantity', function($row) {
                return number_format(abs($row->quantity), 2);
            })
            ->addColumn('subtotal', function($row) {
                return abs($row->quantity) * $row->unit_price_inc_tax;
            })
            ->rawColumns(['payment_status'])
            ->make(true);
    }
