...
class TransactionUtil extends Util
{
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
      ...
      // تحقق أولاً من وجود QR Code في fatora_invoices
$fatora_invoice = \App\Models\FatoraInvoice::where('transaction_id', $transaction_id)
    ->whereNotNull('qr_code')
    ->where('qr_code', '!=', '')
    ->first();

if ($fatora_invoice && !empty($fatora_invoice->qr_code)) {
    // استخدم QR Code من جدول fatora_invoices
    $qr_code_text = $fatora_invoice->qr_code;
        ...
        // add this seprate for sell retuen qr text of zatca
            ...
            $output['show_qr_code'] = ! empty($il->show_qr_code) ? true : false;
            $zatca_qr = ! empty($il->common_settings['zatca_qr']) ? true : false;
            $qr_code_text = null; // Initialize to prevent undefined variable error
            // تحقق أولاً من وجود QR Code في fatora_invoices
$fatora_invoice = \App\Models\FatoraInvoice::where('transaction_id', $transaction_id)
    ->whereNotNull('qr_code')
    ->where('qr_code', '!=', '')
...
if ($fatora_invoice && !empty($fatora_invoice->qr_code)) {
    ...
    $qr_code_text = $fatora_invoice->qr_code;
        ...
        // Get JoFotara QR Code if exists
        $output['fatora_qr_code'] = null;
        $output['fatora_invoice_number'] = null;
        $output['fatora_invoice_uuid'] = null;
        ...
        // dd($fatora_invoice);
        if ($fatora_invoice && !empty($fatora_invoice->qr_code)) {
            // QR code is base64 encoded - prepare it for display
            $output['fatora_qr_code'] = $fatora_invoice->qr_code;
            $output['fatora_invoice_number'] = $fatora_invoice->system_invoice_number;
            $output['fatora_invoice_uuid'] = $fatora_invoice->system_invoice_uuid;
        }
        ...
        foreach ($transaction_lines as $line) {
            ...
            $query = Transaction::join('purchase_lines AS PL', 'transactions.id', '=', 'PL.transaction_id')
                ...
                ->whereIn('transactions.type', ['purchase', 'purchase_transfer',
                    'opening_stock', 'production_purchase','add_quantity' ])
