   ...
   public function getRegisterDetails($register_id = null)
{
    $query = CashRegister::leftjoin(
        ...
        'ct.cash_register_id', '=', 'cash_registers.id'
    )
    ...
    // شروط الوقت والمكان (المنطق الخاص بك)
    $time_cond = "BETWEEN cash_registers.created_at AND IFNULL(cash_registers.closed_at, NOW())";
    $common_filters = "WHERE location_id = cash_registers.location_id AND created_by = cash_registers.user_id AND transaction_date $time_cond";
    $payment_filters = "WHERE tr.location_id = cash_registers.location_id AND tp.created_by = cash_registers.user_id AND tp.paid_on $time_cond";

    // 1. الاستعلام الرئيسي لجلب كافة الحقول
    $register_details = $query->select(
        ...
        DB::raw("(SELECT SUM(IF(tp.method='card' AND tr.type='sell', tp.amount, 0)) - SUM(IF(tp.method='card' AND tr.type='expense', tp.amount, 0)) - SUM(IF(tp.method='card' AND tr.type='sell_return', tp.amount, 0)) FROM transaction_payments AS tp JOIN transactions AS tr ON tp.transaction_id = tr.id $payment_filters) as total_final_card"),

        // المرتجعات والذمم
        DB::raw("SUM(IF(ct.transaction_type='sell_return' , ct.amount, 0)) as total_return"),
        DB::raw("(SELECT SUM(final_total - (SELECT IFNULL(SUM(amount), 0) FROM transaction_payments WHERE transaction_id = transactions.id)) FROM transactions $common_filters AND type = 'sell' AND payment_status != 'paid') as total_due"),

        // تفاصيل المبيعات لكل وسيلة دفع
    ...
    if ($register_details) {
        ...
        $register_details->net_total_cash = ($register_details->cash_in_hand ?? 0) + ($register_details->total_final_cash ?? 0);
    }
