...
class ProductUtil extends Util
{
    ...
    public function getVariationStockDetails($business_id, $variation_id, $location_id)
    {
      ...
      ////////// تعديل لادخال الكميات بصفحة سجل مخزون المنتج  001
    public function getVariationStockHistory($business_id, $variation_id, $location_id)
{
    $stock_history = Transaction::leftjoin('transaction_sell_lines as sl', 'sl.transaction_id', '=', 'transactions.id')
        ...
        // أضفنا quantity_entry هنا لضمان جلبها من قاعدة البيانات
        ->whereIn('transactions.type', [
            'sell', 'purchase', 'stock_adjustment', 'opening_stock', 
            'sell_transfer', 'purchase_transfer', 'production_purchase', 
            'purchase_return', 'sell_return', 'production_sell', 
            'add_quantity'
        ])
        ->select(
    ...
    foreach ($stock_history as $stock_line) {
        $temp_array = [
            ...
            'user_name' => $stock_line->created_by_user,
        ];

        // 1. معالجة نوع "إدخال كميات" (النوع الجديد)
        if ($stock_line->transaction_type == 'add_quantity') {
        ...
        // 4. معالجة تعديل المخزون (stock_adjustment)
        ...
        // 5. معالجة الرصيد الافتتاحي (opening_stock)
        ...
        // 8. معالجة مواد التصنيع (manufacturing ingredient)
        ...
        // 9. معالجة المنتجات المصنعة (manufactured product)
