...
$(document).ready(function() {
 ...
 applyProductOffer(tr, entered_qty); //003
    ...
    ///////////// gift fatora 
   // هذا الكود يربط الـ Checkbox بعملية البيع الأساسية
// 1. عند إرسال النموذج: نجهز بيانات الـ Checkbox
$(document).on('submit', 'form#add_pos_sell_form', function(e) {
    var is_gift = $('#is_gift_receipt').is(':checked') ? 1 : 0;
    $(this).find('input[name="is_gift_receipt"]').remove();
    $(this).append('<input type="hidden" name="is_gift_receipt" value="' + is_gift + '">');
});
   ...
   submitHandler: function(form) {
    ...
    if (cnf) {
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success == 1) {
                    // 1. إخفاء المودال فوراً
                    $('#modal_payment').modal('hide');
                    
                    // 2. إظهار رسالة النجاح أولاً
                    toastr.success(result.msg);
                    ...
                    // 5. الطباعة في النهاية (بعد ظهور الرسالة)
                    if (result.receipt && result.receipt.is_enabled) {
                        // إضافة تأخير بسيط جداً لضمان ظهور الـ Toastr بوضوح
                        setTimeout(function() {
                            pos_print(result.receipt);
                        }, 2500); 
                    }
...
// 🔥🔥🔥 دالة لتطبيق عرض المنتج 🔥🔥🔥
function applyProductOffer(row, quantity) {//003
    var variation_id = row.find('.row_variation_id').val();
    var location_id = $('#location_id').val();
    
    if (!variation_id || !location_id || quantity <= 0) {
        return;
    }
    
    // الحصول على السعر الأصلي من الحقل المخفي
    var hidden_base_price = row.find('input.hidden_base_unit_sell_price').val();
    var multiplier = parseFloat(row.find('input.base_unit_multiplier').val()) || 1;
    var original_price = parseFloat(hidden_base_price) * multiplier;
    
    // إرسال طلب AJAX للحصول على السعر بعد العرض
    $.ajax({
        ...
        success: function(response) {
            if (response.success && response.final_price) {
                var new_price = parseFloat(response.final_price);
                
                // تحديث حقل السعر
                var price_field = row.find('input.pos_unit_price_inc_tax');
                if (price_field.length === 0) {
                    price_field = row.find('input.pos_unit_price');
                }
                
                __write_number(price_field, new_price);
...
function pos_discount(total_amount) {
    ...
    // توزيع الخصم نسبياً على كل منتج
    distribute_discount_on_products(discount, total_amount);

    return discount;
}

function distribute_discount_on_products(total_discount, total_amount) {
    if (total_amount <= 0) return;

    var rows = $('table#pos_table tbody tr');
    ...
    rows.each(function(index) {
        var row = $(this);
        var row_total = __read_number(row.find('input.pos_line_total'));
        var qty = __read_number(row.find('input.pos_quantity'));

        if (total_discount <= 0) {
            __write_number(row.find('input.row_discount_amount'), 0);
            row.find('select.row_discount_type').val('fixed');
            pos_each_row(row);
            var new_total = __read_number(row.find('input.pos_line_total'));
            row.find('span.pos_line_total_text').text(
                __currency_trans_from_en(new_total, false)
            );
            return;
        }

        // نصيب هاد المنتج من الخصم
        var row_discount = 0;
        if (index === row_count - 1) {
            row_discount = total_discount - distributed;
        } else {
            row_discount = (row_total / total_amount) * total_discount;
            distributed += row_discount;
        }

        // ✅ الخصم لكل وحدة (بدون ضريبة) عشان pos_each_row تحسب صح
        var unit_price = __read_number(row.find('input.pos_unit_price'));
        var tax_rate = row.find('select.tax_id').find(':selected').data('rate') || 0;
        var tax_multiplier = 1 + (tax_rate / 100);

        // حوّل الخصم من شامل الضريبة لبدون ضريبة
        var row_discount_ex_tax = qty > 0 ? (row_discount / qty) / tax_multiplier : 0;

        row.find('select.row_discount_type').val('fixed');
        __write_number(row.find('input.row_discount_amount'), row_discount_ex_tax);

        pos_each_row(row);

        var new_line_total = __read_number(row.find('input.pos_line_total'));
        row.find('span.pos_line_total_text').text(
            __currency_trans_from_en(new_line_total, false)
        );

        row.attr('data-row-discount', row_discount.toFixed(4));
    });
...
function pos_print(receipt) {
    if (receipt.html_content != '') {
        if (receipt.print_type === 'browser_silent') {
            // 1. إنشاء إطار مخفي تماماً
            ...
            // حالة التقارير: نفتح نافذة جديدة عادية لتظهر المعاينة
            var reportWin = window.open('', '_blank');
            reportWin.document.write(receipt.html_content);
