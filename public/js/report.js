$(document).ready(function() {
   ...
   if ($('#full_datetime_range').length == 1) {
    $('#full_datetime_range').daterangepicker(
        _.extend({}, dateRangeSettings, {
            timePicker: true,
            timePicker24Hour: false, // تفعيل نظام 12 ساعة
            locale: {
                format: moment_date_format + ' hh:mm A' // hh للوقت، A لـ AM/PM
            }
        }), 
        function(start, end) {
            $('#full_datetime_range').val(
                start.format(moment_date_format + ' hh:mm A') + ' ~ ' + end.format(moment_date_format + ' hh:mm A')
            );
            updateSalesRepresentativeReport(); 
        }
    );

    // إضافة حدث الـ Apply لضمان تحديث القيمة عند الضغط على زر "تطبيق"
    $('#full_datetime_range').on('apply.daterangepicker', function(ev, picker) {
        $(this).val(
            picker.startDate.format(moment_date_format + ' hh:mm A') + ' ~ ' + picker.endDate.format(moment_date_format + ' hh:mm A')
        );
        updateSalesRepresentativeReport();
    });


    $('#full_datetime_range').on('cancel.daterangepicker', function(ev, picker) {
        $(this).val('');
        updateSalesRepresentativeReport();
    });

// تحديث إجمالي المصاريف والمبيعات (الخانات العلوية إن وجدت)
...
if ($('span#sr_total_sales').length > 0) {
    salesRepresentativeTotalSales();
        ...
        data: function(d) {
            ...
            if (picker) {
                ...
                // في حال لم يتحمل الـ picker بعد، نرسل تاريخ اليوم كقيم افتراضية
                d.start_date = moment().startOf('day').format('YYYY-MM-DD HH:mm:ss');
                d.end_date = moment().endOf('day').format('YYYY-MM-DD HH:mm:ss');
            }
       ...
       //Sales representative report -> Sales
        ...
        data: function(d) {
            var picker = $('#full_datetime_range').data('daterangepicker');
            if (picker) {
                // إرسال الوقت بنظام 24 ساعة للخادم لضمان الدقة
                d.start_date = picker.startDate.format('YYYY-MM-DD HH:mm:ss');
                d.end_date = picker.endDate.format('YYYY-MM-DD HH:mm:ss');
            } else {
                // fallback في حال لم يجهز الفلتر بعد (يعطي بيانات اليوم)
                d.start_date = moment().startOf('day').format('YYYY-MM-DD HH:mm:ss');
                d.end_date = moment().endOf('day').format('YYYY-MM-DD HH:mm:ss');
            }
    ...
    data: function(d) {
       ...
       ////////// لاظهار جمنيع المستخدمين بنفس الجدول 001
      $('select#sr_id, select#sr_business_id').change(function() {
    // تحديث الجداول الثلاثة (مبيعات، عمولات، مصاريف)
    updateSalesRepresentativeReport();
    
    // تحديث جدول ملخص المندوبين (الجدول الجديد الذي أضفناه في البداية)
...
////////////////  001  تعديل فلتر الوقت والتاريخ
    var start = moment().startOf('day');
    var end = moment().endOf('day');

    // 2. تهيئة التقويم الجديد مع دعم الوقت ونظام 12 ساعة
    ...
    // ضمان جلب اليوم كاملاً عند اختيار تاريخ من التقويم (تصفير الوقت)
    ...
    // تعيين القيمة الابتدائية في الحقل
    $('#qer_date_filter').val(start.format(moment_date_format + ' hh:mm A') + ' ~ ' + end.format(moment_date_format + ' hh:mm A'));
    ...
    sell_payment_report = $('table#sell_payment_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                // جلب الوقت بصيغة 24 ساعة لإرساله للسيرفر
...
function updateSalesRepresentativeReport() {
    // 1. تحديث جدول الملخص الإجمالي (الجدول الجديد الذي أضفناه )
   if (typeof updateSummaryTable === "function") {
        updateSummaryTable();
    }

    // 2. تحديث الجداول السفلية (DataTables)
    if (typeof sr_sales_report !== 'undefined') { sr_sales_report.ajax.reload(); }
    if (typeof sr_sales_commission_report !== 'undefined') { sr_sales_commission_report.ajax.reload(); }
    if (typeof sr_expenses_report !== 'undefined') { sr_expenses_report.ajax.reload(); }
    
    // 3. تحديث المربعات الإحصائية العلوية (مع فحص الوجود)
    if (typeof salesRepresentativeTotalExpense === "function") { salesRepresentativeTotalExpense(); }
    if (typeof salesRepresentativeTotalSales === "function") { salesRepresentativeTotalSales(); }
    if (typeof salesRepresentativeTotalCommission === "function") { salesRepresentativeTotalCommission(); }

    // 4. تحديث جدول دفعات العمولات
    if ($('#sr_payments_with_commission_table').length > 0 && typeof sr_payments_with_commission_report !== 'undefined') {
        sr_payments_with_commission_report.ajax.reload();
    }
}

function salesRepresentativeTotalExpense() {
    // التعديل: استخدام المعرف الجديد وإرسال الوقت HH:mm:ss
    var start = $('#full_datetime_range').data('daterangepicker').startDate.format('YYYY-MM-DD HH:mm:ss');
    var end = $('#full_datetime_range').data('daterangepicker').endDate.format('YYYY-MM-DD HH:mm:ss');
...
function salesRepresentativeTotalSales() {
    // التعديل هنا: استخدام المعرف الجديد وإضافة الوقت ليتوافق مع الكنترولر
    var start = $('#full_datetime_range').data('daterangepicker').startDate.format('YYYY-MM-DD HH:mm:ss');
    var end = $('#full_datetime_range').data('daterangepicker').endDate.format('YYYY-MM-DD HH:mm:ss');
...
function salesRepresentativeTotalCommission() {
    // التعديل هنا: استخدام المعرف الجديد وإضافة الوقت
    var start = $('#full_datetime_range').data('daterangepicker').startDate.format('YYYY-MM-DD HH:mm:ss');
    var end = $('#full_datetime_range').data('daterangepicker').endDate.format('YYYY-MM-DD HH:mm:ss');
    ...
    // ... بقية كود الـ AJAX كما هو بدون تغيير ...
    ...
    if (data_sell.commission_agent) {
        ...
        $.ajax({
            ...
            data: data_sell,
            success: function(data) {
                // الكود الداخلي لطباعة النتائج لا يتغير
