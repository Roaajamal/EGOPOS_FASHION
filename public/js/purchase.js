    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
               ...
               // التعديل هنا لاستخدام الـ ID الجديد وجلب الوقت بالكامل
        var start = '';
        var end = '';
        if ($('#qer_date_filter').val()) {
            var picker = $('#qer_date_filter').data('daterangepicker');
            // نرسل التنسيق الكامل 24 ساعة مع الثواني لضمان دقة البحث في قاعدة البيانات
            start = picker.startDate.format('YYYY-MM-DD HH:mm:00');
            end = picker.endDate.format('YYYY-MM-DD HH:mm:59');
        }
        d.start_date = start;
        d.end_date = end;
