...
if ($('#search_product').length) {
    ...
    const processBarcode = () => {
        if(barcodeBuffer.trim() !== ''){
            ...
            input.val(barcodeBuffer.trim());

            // تشغيل Autocomplete
            input.autocomplete("search", barcodeBuffer.trim());

            // بعد قليل، اختر أول عنصر تلقائيًا
            setTimeout(() => {
                let menu = input.autocomplete("widget");
                if(menu.find("li").length > 0){
                    menu.find("li").first().click(); // يختار أول عنصر
                }
            }, 50); // 50ms كافية لإظهار النتائج
    ...
    $('#search_product').on('input', function(e){
        ...
        barcodeTimer = setTimeout(processBarcode, 120); // 120ms buffer
    });
