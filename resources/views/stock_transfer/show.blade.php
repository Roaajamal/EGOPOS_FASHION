				      ...
				      <table class="table bg-gray">
				        <tr class="bg-green">
				          ...
				          @if(!empty($p_labels['custom_field_3']))
				              <th class="custom-field-3">{{ $p_labels['custom_field_3'] }}</th>
				          @endif
				          @if(!empty($p_labels['custom_field_1']))
				              <th class="custom-field-1">{{ $p_labels['custom_field_1'] }}</th>
				          @endif
				          @if(!empty($p_labels['custom_field_2']))
				              <th class="custom-field-2">{{ $p_labels['custom_field_2'] }}</th>
				          @endif
				          <th>@lang('sale.qty')</th>
				        ...
				        @foreach($sell_transfer->sell_lines as $sell_lines)
				          <tr>
				            ...
				            @if(!empty($p_labels['custom_field_3']))
				                <td class="custom-field-3">{{ $sell_lines->product->product_custom_field3 ?? '-' }}</td>
				            @endif
				            @if(!empty($p_labels['custom_field_1']))
				                <td class="custom-field-1">{{ $sell_lines->product->product_custom_field1 ?? '-' }}</td>
				            @endif
				            @if(!empty($p_labels['custom_field_2']))
				                <td class="custom-field-2">{{ $sell_lines->product->product_custom_field2 ?? '-' }}</td>
				            @endif
				    ...
				    <div class="table-responsive">
				      <table class="table">
				        ...
				        <tr class="@cannot('view_purchase_price') show_price_with_permission no-print @endcan">
				          ...
				          <td></td>
				          <td><span class="display_currency pull-right" data-currency_symbol="true">{{ @num_format($total) }}</span></td>
				        </tr>
				        ...
				        <tr class="@cannot('view_purchase_price') show_price_with_permission no-print @endcan">
				          ...
				          <td></td>
				          <td><span class="display_currency pull-right" data-currency_symbol="true">{{ @num_format($sell_transfer->final_total) }}</span></td>
				        </tr>
				      </table>
				    </div>
				</div>
