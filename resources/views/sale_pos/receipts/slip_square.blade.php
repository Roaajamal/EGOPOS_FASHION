...
<html lang="en">
    ...
    @endif
			...
			@endif
           <table style="width: 100%; margin-top: 15px !important; table-layout: fixed; border-collapse: collapse; border: 2px solid #000;" class="table-f-12 mb-10">
    <thead>
        <tr style="background: #ece6e6; color: #161414;">
            <th class="serial_number" style="border: 1px solid #000; padding: 5px 4px;">#</th>
            <th class="description" width="30%" style="border: 1px solid #000; padding: 5px 4px;">
                {{$receipt_details->table_product_label}}
            </th>
            <th class="quantity text-right" style="border: 1px solid #000; padding: 5px 4px;">
                {{$receipt_details->table_qty_label}}
            </th>
           
                
                 @if(!empty($receipt_details->show_price))
            <th class="unit_price text-right" style="border: 1px solid #000; padding: 5px 4px;">
            ...
            @if(!empty($receipt_details->discounted_unit_price_label))
                <th class="text-right" style="border: 1px solid #000; padding: 5px 4px;">
                    {{$receipt_details->discounted_unit_price_label}}
                </th>
            @endif
            @if(!empty($receipt_details->item_discount_label))
                <th class="text-right" style="border: 1px solid #000; padding: 5px 4px;">{{$receipt_details->item_discount_label}}</th>
            @endif
            <th class="price text-right" style="border: 1px solid #000; padding: 5px 4px;">{{$receipt_details->table_subtotal_label}}</th>
    ...
    <tbody>
        @forelse($receipt_details->lines as $line)
            <tr style="{{ $loop->even ? 'background:#f9f9f9;' : '' }}">
                <td class="serial_number" style="vertical-align: top; border: 1px solid #333; padding: 5px 4px;">
                    {{$loop->iteration}}
                </td>
                <td class="description" style="border: 1px solid #333; padding: 5px 4px;">
                ...
                <td class="quantity text-right" style="border: 1px solid #333; padding: 5px 4px; vertical-align: top;">
                    {{$line['quantity']}} 
   ...
   @if(!empty($receipt_details->show_unit))
    ...
    @endif
               ...
               @if(!empty($receipt_details->show_price))
                <td class="unit_price text-right" style="border: 1px solid #333; padding: 5px 4px; vertical-align: top;">
                ...
                @if(!empty($receipt_details->discounted_unit_price_label))
                    <td class="text-right" style="border: 1px solid #333; padding: 5px 4px; vertical-align: top;">
                ...
                @endif
                <td class="price text-right" style="border: 1px solid #333; padding: 5px 4px; vertical-align: top;">{{$line['line_total']}}</td>
            ...
            @if(!empty($line['modifiers']))
                @foreach($line['modifiers'] as $modifier)
                    <tr style="background: #f0f0f0;">
                        ...
                        <td style="border: 1px solid #333; padding: 5px 4px;">
                            {{$modifier['name']}} {{$modifier['variation']}} 
                            @if(!empty($modifier['sub_sku'])), {{$modifier['sub_sku']}} @endif @if(!empty($modifier['cat_code'])), {{$modifier['cat_code']}}@endif
                            @if(!empty($modifier['sell_line_note']))({!!$modifier['sell_line_note']!!}) @endif 
                        </td>
                        <td class="text-right" style="border: 1px solid #333; padding: 5px 4px;">{{$modifier['quantity']}} {{$modifier['units']}}</td>
                        @if(empty($receipt_details->hide_price))
                        <td class="text-right" style="border: 1px solid #333; padding: 5px 4px;">{{$modifier['unit_price_inc_tax']}}</td>
                        @if(!empty($receipt_details->discounted_unit_price_label))
                            <td class="text-right" style="border: 1px solid #333; padding: 5px 4px;">{{$modifier['unit_price_exc_tax']}}</td>
                        @endif
                        @if(!empty($receipt_details->item_discount_label))
                            <td class="text-right" style="border: 1px solid #333; padding: 5px 4px;">0.00</td>
                        @endif
                        <td class="text-right" style="border: 1px solid #333; padding: 5px 4px;">{{$modifier['line_total']}}</td>
                        @endif
                    </tr>
                @endforeach
            @endif
...
/* طباعة QZ فقط - ما بتأثر على الطباعة العادية */
body.qz-print * {
    font-size: 11px !important;
    line-height: 1.3 !important;
}
body.qz-print .ticket {
