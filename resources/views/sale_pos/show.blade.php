...
<div class="modal-body">
  ...
  <div class="modal-footer">
    ...
    {{-- زر إرسال الفاتورة لنظام الفوترة الأردني --}}
    @if($sell->type != 'sales_order')
      <button type="button" class="tw-dw-btn tw-dw-btn-warning tw-text-white send-to-fatora" data-transaction-id="{{$sell->id}}" id="send_to_fatora_btn_{{$sell->id}}">
        <i class="fa fa-paper-plane" aria-hidden="true"></i> إرسال للفوترة الأردنية
      </button>
      <span id="fatora_status_{{$sell->id}}" style="margin-left: 10px;"></span>
    @endif
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    // Check invoice status on modal open
    var transactionId = '{{$sell->id}}';
    checkFatoraStatus(transactionId);

    // Send to Fatora button click
    $(document).on('click', '.send-to-fatora', function(e){
      e.preventDefault();
      var btn = $(this);
      var transactionId = btn.data('transaction-id');
      
      
      if(confirm('هل أنت متأكد من إرسال هذه الفاتورة إلى نظام الفوترة الأردني؟')) {
        btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> جاري الإرسال...');
        
        $.ajax({
          url: '{{url("/fatora/send-invoice")}}',
          method: 'POST',
          data: {
            _token: '{{csrf_token()}}',
            transaction_id: transactionId,
            invoice_type: 'income', // تغيير من general_sales إلى income
            payment_method: 'cash'
          },
          dataType: 'json',
          success: function(response) {
            if(response.success) {
              ...
              swal('خطأ!', response.message || 'فشل إرسال الفاتورة', 'error');
            }
            btn.prop('disabled', false).html('<i class="fa fa-paper-plane"></i> إرسال للفوترة الأردنية');
          },
          error: function(xhr) {
            var errorMsg = 'حدث خطأ أثناء إرسال الفاتورة';
    ...
    // Check Fatora status function
    function checkFatoraStatus(transactionId) {
        ...
        success: function(response) {
          if(response.success && response.data) {
            ...
            if(status === 'sent' || status === 'accepted') {
              statusHtml = '<span class="badge badge-success"><i class="fa fa-check"></i> تم الإرسال</span>';
              $('#send_to_fatora_btn_' + transactionId).prop('disabled', true).addClass('tw-dw-btn-secondary').removeClass('tw-dw-btn-warning');
            ...
            $('#fatora_status_' + transactionId).html(statusHtml);
            
            // Show QR Code if available
            if(response.data.qr_code) {
              // You can add QR code display logic here
              console.log('QR Code available:', response.data.qr_code);
            }
          }
        }
      });
