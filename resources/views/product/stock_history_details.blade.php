	...
	<div class="col-md-12">
		...
		<table class="table table-slim" id="stock_history_table">
			...
			<tr>
				...
				<th>@lang('lang_v1.user')</th>  <!--  add user name 001-->
			</tr>
			</thead>
			<tbody>
			@forelse($stock_history as $history)
				<tr>
					<td>  <!-- اذا كان ادخال كميات تغير الحالة الى ادخال كميات 001-->
                    @if(!empty($history['ref_no']) && strpos($history['ref_no'], 'QE') === 0)
                    {{-- هنا نغير الاسم فقط لـ QE --}}
                    إدخال كميات
                    @else
                    {{-- هنا نعرض الاسم القادم من الكنترولر كما هو (مشتريات لـ PO أو مبيعات.. إلخ) --}}
                    {{$history['type_label']}}
                    @endif
                    </td>  <!--     001   -->
					...
					<td>
                        {{ $history['user_name'] ?? '--' }}  <!-- add user name 001    -->
                    </td>
				</tr>
			@empty
