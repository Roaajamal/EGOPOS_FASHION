...
Route::middleware('web', 'SetSessionData', 'auth', 'language', 'timezone', 'AdminSidebarMenu')->prefix('accounting')->group(function () {
    ...
    Route::get('reports/income-statement', [\Modules\Accounting\Http\Controllers\ReportController::class, 'getIncomeStatement'])->name('accounting.incomeStatement');
    ...
    // ---  Edit ---
    Route::get('transactions/get-bulk-map-modal', [\Modules\Accounting\Http\Controllers\TransactionController::class, 'getBulkMapModal']);
    Route::post('transactions/save-bulk-map', [\Modules\Accounting\Http\Controllers\TransactionController::class, 'saveBulkMap']);
    // --------------------------------


    // السندات 
    Route::resource('vouchers', \Modules\Accounting\Http\Controllers\VoucherController::class);
    // مسار خاص للطباعة
     Route::get('vouchers/{id}/print',[\Modules\Accounting\Http\Controllers\VoucherController::class, 'print'])->name('vouchers.print');
    ...
    //جدول الشيكات
    Route::group(['middleware' => ['web', 'auth', 'language'], 'prefix' => 'accounting'], function () {
    Route::get('/cheques',[\Modules\Accounting\Http\Controllers\ChequeController::class, 'index'])->name('accounting.cheques');
    Route::post('/cheques/update-status', [\Modules\Accounting\Http\Controllers\ChequeController::class, 'updateStatus'])->name('accounting.cheques.updateStatus');
    Route::get('/cheques/print/{id}',[\Modules\Accounting\Http\Controllers\ChequeController::class, 'print'])->name('accounting.cheques.print');
});