...
{
    if (request()->ajax()) {
        // تصحيح: تعريف المتغير أولاً
        $datatable = request()->input('datatable');

        if ($datatable == 'payment') {
            return $this->_allPayments();
        }
        if ($datatable == 'sell') {
            return $this->_allSales();
        }
        if ($datatable == 'purchase') {
            return $this->_allPurchases();
        }
        if ($datatable == 'expense') {
            return $this->_allExpenses();
        }

        // إضافة المخزون الافتتاحي
        if ($datatable == 'opening_stock') {
            return $this->_allOpeningStock();
        }

        // إضافة إدخال الكميات (تسوية المخزون)
        if ($datatable == 'stock_adjustment' || $datatable == 'add_quantity') {
           return $this->_allStockAdjustments();
        }
    }
    ...
    protected function _allSales()
    {
        ...
        $rawColumns = ['final_total', 'action', 'total_paid', 'total_remaining', 
                   'payment_status', 'invoice_no', 'discount_amount', 'tax_amount', 
                   'total_before_tax', 'shipping_status', 'types_of_service_name', 
   ...
   protected function _allOpeningStock()
{
    $business_id = request()->session()->get('user.business_id');
    
    // تصحيح: إضافة Join لجلب الموقع والمستخدم ليتناسب مع الـ JS
    ...
    return $this->_makeTransactionDatatable($query, 'opening_stock');
}

protected function _allStockAdjustments()
{
    $business_id = request()->session()->get('user.business_id');
    
    // تصحيح: إضافة الحقول المطلوبة لجدول التسوية
    $query = Transaction::where('transactions.business_id', $business_id)
        ...
        ->select([
            ...
            'transactions.transaction_date', 
            'transactions.final_total',
            'transactions.adjustment_type', // مطلوب في جدول التسوية
            'transactions.total_amount_recovered', // مطلوب في جدول التسوية
            'transactions.additional_notes',
            'bl.name as business_location',
            DB::raw("CONCAT(COALESCE(u.first_name, ''), ' ', COALESCE(u.last_name, '')) as added_by")
        ]);

    return $this->_makeTransactionDatatable($query, 'add_quantity');
}

/**
 * فتح مودال الربط (الفردي والجماعي)
 */
public function map(Request $request)
{
    ...
    if (request()->ajax()) {
        ...
        // تنظيف الـ IDs
        if (is_array($id)) {
            $id = array_filter($id, function($val) {
        ...
        if (empty($id)) {
            return '<div class="modal-dialog"><div class="modal-content"><div class="modal-body text-danger text-center">لم يتم التعرف على معرفات العمليات. يرجى إعادة تحميل الصفحة.</div></div></div>';
        }

        // جلب قائمة الحسابات (ضروري جداً لظهور الحقول في المودال)
        $accounts = \Modules\Accounting\Entities\AccountingAccount::forDropdown($business_id);

        // --- حالة الربط الجماعي (إذا كان المرسل مصفوفة) ---
        if (is_array($id)) {
            $transaction_ids = $id;
            $default_payment_account = null;
            $default_deposit_to = null;
            $note = null;
            return view('accounting::transactions.map')
                ->with(compact('transaction_ids', 'type', 'accounts', 'default_payment_account', 'default_deposit_to', 'note'));
        }
        ...
        if (in_array($type, ['sell_payment', 'purchase_payment'])) {
            $transaction_payment = \App\TransactionPayment::where('id', $id)
                ->where('business_id', $business_id)
                ->firstOrFail();
            
            $existing_payment = \Modules\Accounting\Entities\AccountingAccountsTransaction::where('transaction_payment_id', $id)
                ->where('map_type', 'payment_account')->first();
            $existing_deposit = \Modules\Accounting\Entities\AccountingAccountsTransaction::where('transaction_payment_id', $id)
                ->where('map_type', 'deposit_to')->first();
        } else {
            // يشمل (sell, purchase, expense, opening_stock, add_quantity)
            $transaction = \App\Transaction::where('id', $id)
                ->where('business_id', $business_id)

                ->whereIn('type', ['sell', 'purchase', 'expense', 'opening_stock', 'add_quantity', 'stock_adjustment'])
                ->firstOrFail();

            $existing_payment = \Modules\Accounting\Entities\AccountingAccountsTransaction::where('transaction_id', $id)
                ->where('map_type', 'payment_account')->first();
            $existing_deposit = \Modules\Accounting\Entities\AccountingAccountsTransaction::where('transaction_id', $id)
                ->where('map_type', 'deposit_to')->first();
        }

        $default_payment_account = !empty($existing_payment) ? \Modules\Accounting\Entities\AccountingAccount::find($existing_payment->accounting_account_id) : null;
        $default_deposit_to = !empty($existing_deposit) ? \Modules\Accounting\Entities\AccountingAccount::find($existing_deposit->accounting_account_id) : null;
        $note = !empty($existing_deposit) ? $existing_deposit->note : null;

        return view('accounting::transactions.map')
            ->with(compact('transaction', 'transaction_payment', 'type', 'accounts', 'default_payment_account', 'default_deposit_to', 'note'));
    }
}
...
public function saveMap(Request $request)
{
    ...
    try {
        if (request()->ajax()) {
            ...
            $note = $request->get('note'); // تأكد أن الاسم في الفرونت آند note أو description

            // تحويل المعرفات لمصفوفة دائماً
            $transaction_ids = is_array($ids) ? $ids : [$ids];

            foreach ($transaction_ids as $id) {
                if (empty($id) || $id == 'undefined') continue;

                // استدعاء الـ Utility بالترتيب الصحيح
                $this->accountingUtil->saveMap(
                    $type,            // 1. النوع
                    $id,              // 2. المعرف
                    $user_id,         // 3. المستخدم
                    $business_id,     // 4. البزنس
                    $deposit_to,      // 5. حساب الإيداع (مدين)
                    $payment_account, // 6. حساب الدفع (دائن)
                    $note             // 7. الملاحظة
                );
            }
    ...
    //new edit 
    /**
     * عرض مودال الربط الجماعي للفواتير المختارة
     */
    public function getBulkMapModal(Request $request)
    {
        $business_id = request()->session()->get('user.business_id');
        ...
        if (request()->ajax()) {
            $transaction_ids = $request->get('ids'); // مصفوفة الـ IDs المرسلة من الجافاسكربت
            
            return view('accounting::transactions.bulk_map')
                    ->with(compact('transaction_ids'));
        }
    }

    /**
     * حفظ الربط الجماعي للفواتير بالحسابات المحددة
     */
    public function saveBulkMap(Request $request)
    {
        $business_id = request()->session()->get('user.business_id');
        ...
        try {
            if (request()->ajax()) {
                ...
                // التكرار على كل ID تم تحديده وتنفيذ الربط له
                foreach ($transaction_ids as $id) {
                    // نستخدم الدالة الموجودة مسبقاً في الـ Utility لضمان نفس منطق الحفظ
                    $this->accountingUtil->saveMap('sell', $id, $user_id, $business_id, $deposit_to, $payment_account, $note);
                }
...
protected function _makeTransactionDatatable($query, $type)
{
    return Datatables::of($query)
        ->addColumn('action', function ($row) use ($type) {
            ...
            if (auth()->user()->can('accounting.map_transactions')) {
                ...
                $label = $is_mapped ? __('accounting::lang.edit_mapping') : __('accounting::lang.map_transaction');
                $btn_class = $is_mapped ? 'tw-dw-btn-warning' : 'tw-dw-btn-primary';

                $html .= '<a href="#" 
                    data-href="'.action([\Modules\Accounting\Http\Controllers\TransactionController::class, 'map']).'?id='.$row->id.'&type='.$type.'" 
                    class="btn-modal tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline '.$btn_class.' tw-w-max" 
                    data-container=".view_modal"><i class="fas fa-link"></i> '.$label.'</a>';
            }
            return $html;
        })
        ->editColumn('transaction_date', '{{@format_datetime($transaction_date)}}')
