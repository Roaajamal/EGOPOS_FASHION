...
class AccountingAccountsTransaction extends Model
{
    ...
    // إضافة علاقة مع الفرع (BusinessLocation)
    ...
    public static function createTransaction($data)
    {
        ...
        // إضافة الـ location_id هنا
        $transaction->location_id = ! empty($data['location_id']) ? $data['location_id'] : null;
        
        $transaction->transaction_id = ! empty($data['transaction_id']) ? $data['transaction_id'] : null;
        $transaction->transaction_payment_id = ! empty($data['transaction_payment_id']) ? $data['transaction_payment_id'] : null;
        ...
        $transaction->save();
        return $transaction; // يفضل إرجاع الكائن نفسه
    }
    ...
    public static function updateOrCreateMapTransaction($data)
    {
        $transaction = AccountingAccountsTransaction::updateOrCreate(
            ...
            [
                ...
                'location_id' => ! empty($data['location_id']) ? $data['location_id'] : null, // إضافة الفرع هنا
                'amount' => $data['amount'],
                'type' => $data['type'], 
